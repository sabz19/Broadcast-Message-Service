...
public class Node {
	...
	Logger logger,sum_logger,children_logger;
	FileHandler fh,fs,ch;
	...
	public Node(int num_nodes, int node_id, int port,String host_names,String neighbor_hosts,
		...
		ch = new FileHandler("/home/012/s/sx/sxn164530/Project/Logs/Children/logs"+node_id+".log");
		...
		children_logger = Logger.getLogger("Children");
		children_logger.setLevel(Level.INFO);
		children_logger.addHandler(ch);
	...
	/*
	 * Updates for broadcast message
	 */
	
	private void writeChildren(){
		children_logger.info("My children");
		for(Integer child : spanning_tree_children){
			children_logger.info(Integer.toString(child));
		}
	}
	...
	/*
	 * Broadcast Messages to children
	 */
			...
			if(counter == 1){
				...
				synchronized (broad_cast_converge_cast_lock){
					try{
						...
						Thread.sleep((long) exp_distribution[counter]);
	...
	public static void main(String[] args) throws IOException, SecurityException, InterruptedException {
		...
		exp_distribution = new double[num_broadcast+1];
